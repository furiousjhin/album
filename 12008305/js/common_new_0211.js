...
var Commonjs = Commonjs || (function () {
	...
	Canvas.prototype.handleFileLoad = function (image) {
		...
		this.counter++;
		if (this.counter >= this.count_max) this.handleComplete();
	}
	...
	function PopupTemplate(doc, default_param) {
		...
		if (this.doc instanceof $) this.doc_type = "jQuery";
		else this.doc_type = typeof this.doc;
	};
	...
	function Popup(id, template) {
		...
		if (Popup._mask == null) {
			if ($("#ibox_mask").size() > 0) Popup._mask = $("#ibox_mask");
			else {
	...
	Popup._parse_val = function (target) {
		if (target instanceof $) return target.prop("outerHTML");
		else return target;
	};
	Popup.prototype.get_contents = function (type) {
		var _type = type || "jQuery";
		if (_type == "jQuery") return this._dom;
		else return this._dom.prop("outerHTML");
	}
	...
	Popup.prototype.dispatchEvent = function (event_names) {
		...
		if (targets) {
			for (var i = 0; i < targets.length; i++) targets[i](this);
		}
	};
	Popup.prototype.on = function (event_name, callback) {
		if (this._listeners[event_name] === undefined) this._listeners[event_name] = [];
	...
	*/
	...
	Popup.prototype.open = function () {
		...
		if (this._state == Popup.enum_state.OPEN) return this;
		this._state = Popup.enum_state.OPEN;
		if (document.doctype) window_height = $(window).height();
		else window_height = window.innerHeight;
		...
		if (this._dom.height() * zoom > window_height) position_top = $(window).scrollTop() + 20;
		else position_top = (window_height - this._dom.height() * zoom) / 2 + $(window).scrollTop();
		position_top /= zoom;
	...
	Popup.prototype.close = function (mask_off) {
		...
		if (event && event.currentTarget && event.currentTarget.id == "ibox_mask") closable = this._closable;
		else closable = true;
		if (this._state == Popup.enum_state.CLOSE || !closable) return;
		this._state = Popup.enum_state.CLOSE;
		this._dom.detach();
		// マスクOFFしない場合はマスク消さない
		if (_mask_off) {
	...
	return {
		...
		*/
		setTab: function (param) {
			...
			$tabs.on("click", function (e, is_init) {
				if (!is_init && $(this).hasClass("selected")) return;
				tab_change(this);
				func(this);
			});
		...
		*/
		getFormatTime: function (msecond, format, is_short) {
			...
			if (format.match(/hh/)) h = "" + ("0" + h).slice(-2);
			if (format.match(/mm/)) m = "" + ("0" + m).slice(-2);
			if (format.match(/ss/)) s = "" + ("0" + s).slice(-2);
			if (is_short) {
				if (+h == 0) format = format.replace(/^.*?m/, "m")
			}
